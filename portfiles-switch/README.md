...
Their job is to satisfy impacto's `vcpkg.json` dependency graph, preferring devkitPro's
own copies of libraries where possible — the technique from
...
source for the Switch triplet just like on desktop. `openal-soft` and `sdl3` also
build from source rather than reuse devkitPro's copy — see flavour 3 below.
...
| `ffmpeg`    | `share/ffmpeg/FFMPEGConfig.cmake` | wraps devkitPro's `libav*`/`switch-dav1d` |
