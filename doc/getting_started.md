...
```
-g <game_profile_name>            # Specify active game/profile
-p <patch_profile_name>           # Enables patch override
-ll <log_level>                   # Set log level (Fatal, Warning, Info, Debug)
-lc <log_channel>                 # Enable specific log channel (repeat flag to specify multiple)
-lf <log_file_path>               # Specify Log File Path
-bp <custom_basepaths.lua>        # Use custom path to basepaths config
-gc <custom_gamedefinitions.lua>  # Use custom path to game definitions config
-uc <>./custom_user_config.lua>   # Use custom path to user config
```
