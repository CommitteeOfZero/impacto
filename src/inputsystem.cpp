...
static std::array<std::optional<TouchState>, FingerTapMax> CurrentFingers{};
...
static void HandleTaps(SDL_TouchFingerEvent const& fingerUpEvent, bool tappable,
                       TouchState const& liftedFinger) {
  using namespace std::chrono;
  assert(fingerUpEvent.type == SDL_EVENT_FINGER_UP);
  const nanoseconds now = nanoseconds(fingerUpEvent.timestamp);
...
static void HandleFlicks(SDL_TouchFingerEvent const& fingerUpEvent,
                         bool tappable, TouchState const& liftedFinger) {
  using namespace std::chrono;
  ...
  const nanoseconds elapsedTime =
      nanoseconds(fingerUpEvent.timestamp) - liftedFinger.StartTime;

  const glm::vec2 liftedPos = SDLMouseCoordsToDesign(
      (int)(fingerUpEvent.x * (float)Window->WindowWidth),
      (int)(fingerUpEvent.y * (float)Window->WindowHeight));
