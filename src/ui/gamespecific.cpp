...
void RenderLayer(uint32_t layer) {
  switch (Profile::GameSpecific::GameSpecificType) {
    case +GameSpecificType::CHLCC: {
      if (ScrWork[SW_RENDERTARGET_PRI0] == static_cast<int>(layer) &&
          ScrWork[SW_RENDERTARGET_BUF0] != 0) {
        Renderer->SetFramebuffer(ScrWork[SW_RENDERTARGET_BUF0] / 2);
      }
      if (ScrWork[SW_RENDERTARGET_PRI1] == static_cast<int>(layer) &&
          ScrWork[SW_RENDERTARGET_BUF1] != 0) {
        Renderer->SetFramebuffer(ScrWork[SW_RENDERTARGET_BUF1] / 2);
      }
