...
Model* Model::Load(uint32_t modelId) {
  ...
  stream->Seek(ModelFileCountsOffset, IoSeek::Set);
  ...
  for (uint32_t i = 0; i < result->MeshCount; i++) {
    ...
    stream->Seek(seekPos, IoSeek::Set);
  ...
  for (uint32_t i = 0; i < result->MeshCount; i++) {
    stream->Seek(MeshInfosOffset + meshInfoSize * i, IoSeek::Set);
  ...
  for (uint32_t i = 0; i < result->MeshCount; i++) {
    ...
    // Read vertex buffers
    stream->Seek(RawVertexOffset, IoSeek::Set);
    if (result->Type == ModelType_Character) {
      if (Profile::Scene3D::Version == LKMVersion::DaSH) {
        for (uint32_t j = 0; j < mesh->VertexCount; j++) {
          ...
          // vec3 unk1, vec4 unk2
          stream->Seek(7 * sizeof(float), IoSeek::Cur);
          // UV
          ...
          if (mesh->UsedBones > 0) {
            ...
            *(int*)vertex->BoneIndices = 0;
            ...
            stream->Seek(4 * sizeof(uint8_t) + 4 * sizeof(float), IoSeek::Cur);
          }
        }
      } else {
        for (uint32_t j = 0; j < mesh->VertexCount; j++) {
    ...
    // Read indices
    stream->Seek(RawIndexOffset, IoSeek::Set);
  ...
  for (uint32_t i = 0; i < result->MorphTargetCount; i++) {
    ...
    // Read vertex buffers
    stream->Seek(RawMorphVertexOffset, IoSeek::Set);
  ...
  // Read textures
  stream->Seek(TexturesOffset, IoSeek::Set);
