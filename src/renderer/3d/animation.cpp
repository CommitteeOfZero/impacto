...
Target GetTarget(Stream* stream, Model* model) {
  ...
  if (Profile::Scene3D::Version == LKMVersion::DaSH) {
    ...
    stream->Seek(2 * sizeof(uint16_t), IoSeek::Cur);
...
ModelAnimation* ModelAnimation::Load(Stream* stream, Model* model,
  ...
  stream->Seek(HeaderDurationOffset, IoSeek::Set);
  result->Duration =
  ...
  for (uint32_t i = 0; i < trackCount; i++) {
    ...
    stream->Seek(tracksOffset + trackSize * i, IoSeek::Set);
    ...
    if (target.Type == TargetType_Bone) {
      ...
      // Skip rotates
      stream->Seek(3 * sizeof(uint16_t), IoSeek::Cur);
  ...
  for (uint32_t i = 0; i < trackCount; i++) {
    ...
    if (target.Type == TargetType_Bone) {
      ...
      stream->Seek(rawRotXOffset, IoSeek::Set);
      ReadArrayLE((float*)rotXBuffer, stream, 2 * rawRotXCount);
      stream->Seek(rawRotYOffset, IoSeek::Set);
      ReadArrayLE((float*)rotYBuffer, stream, 2 * rawRotYCount);
      stream->Seek(rawRotZOffset, IoSeek::Set);
  ...
  for (uint32_t i = 0; i < trackCount; i++) {
    ...
    if (target.Type == TargetType_Bone) {
      ...
      for (int j = STT_Visibility; j <= STT_ScaleZ; j++) {
        ...
        stream->Seek(HeaderSize + currentKeyframeOffset, IoSeek::Set);
      ...
      stream->Seek(HeaderSize + rawVisibilityOffset, IoSeek::Set);
      ...
      for (int j = 0; j < morphTargetCount; j++) {
        stream->Seek(HeaderSize + rawInfluenceOffsets[j], IoSeek::Set);
        ReadArrayLE((float*)(result->CoordKeyframes + currentCoordOffset),
