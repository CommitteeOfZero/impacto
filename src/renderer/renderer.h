...
class IRenderer {
 ...
 private:
  ...
  virtual void DrawSpriteImpl(Sprite const& sprite, RectF const& dest,
                          ...
                          bool inverted, bool isScreencap) = 0;
  virtual void DrawSpriteImpl(Sprite const& sprite, glm::vec2 topLeft,
                          glm::vec4 tint, glm::vec2 scale, float angle,
                          bool inverted, bool isScreencap) = 0;
  virtual void DrawRectImpl(RectF const& dest, glm::vec4 color, float angle) = 0;
  ...
  virtual void DrawCCMessageBoxImpl(Sprite const& sprite, Sprite const& mask,
                                RectF const& dest, glm::vec4 tint, int alpha,
                                int fadeRange, float effectCt, bool isScreencap) = 0;

  virtual void DrawSprite3DRotatedImpl(Sprite const& sprite, RectF const& dest, float depth,
                                   glm::vec2 vanishingPoint, bool stayInScreen, glm::quat rot,
                                   glm::vec4 tint, bool inverted) = 0;
  virtual void DrawSprite3DRotatedImpl(Sprite const& sprite, glm::vec2 topLeft, float depth,
                                   glm::vec2 vanishingPoint, bool stayInScreen,
                                   glm::quat rot, glm::vec4 tint, glm::vec2 scale, bool inverted) = 0;
  virtual void DrawRect3DRotatedImpl(RectF const& dest, float depth, glm::vec2 vanishingPoint,
                                 bool stayInScreen, glm::quat rot, glm::vec4 color) = 0;

  virtual void DrawProcessedTextImpl(ProcessedTextGlyph* text, int length, Font* font,
                                 float opacity, bool outlined, bool smoothstepGlyphOpacity) = 0;

  virtual void DrawCharacterMvlImpl(Sprite const& sprite, glm::vec2 topLeft,
                                int verticesCount, float* mvlVertices, int indicesCount,
                                uint16_t* mvlIndices, bool inverted, glm::vec4 tint) = 0;

  virtual void DrawVideoTextureImpl(YUVFrame const& tex, RectF const& dest,
                                glm::vec4 tint, float angle, bool alphaVideo) = 0;
  virtual void DrawVideoTextureImpl(YUVFrame const& tex, glm::vec2 topLeft,
                                glm::vec4 tint, glm::vec2 scale, float angle,
                                bool alphaVideo) = 0;
