...
void DialoguePage::AddString(Vm::Sc3VmThread* ctx, Audio::AudioStream* voice,
  ...
  if (HasName) {
    uint32_t oldIpOffset = ctx->IpOffset;
...
void InitNamePlateData(Vm::Sc3Stream& stream) {
  while (stream.PeekU8() == 0xFFFF) {
    uint16_t id = stream.ReadU8();
    uint16_t stringId = stream.ReadU8();
    uint32_t nameAddr =
