// ScrWork
V(SW_TITLECT)
V(LR_DATE)
V(SW_MENUCT)
V(SW_MOVIEMODE_ALPHA)
V(SW_MOVIEMODE_CUR)
V(SW_ACTORVOICE_ALPHA)
V(SW_ACTORVOICE_CUR)
V(SW_ALBUM_ALPHA)
V(SW_ALBUM_LOADFILE)
V(SW_ALBUM_LOADBUF)
V(SW_PLAYDATA_ALPHA)
V(SW_MUSICMODE_ALPHA)
V(SW_OPTIONALPHA)
V(SW_FILEALPHA)
V(SW_TIPSALPHA)
V(SW_TOTALPLAYTIME)
V(SW_CHARACTERIDMAPPING)
V(SW_ANIME0CHANO)
V(SW_ANIME1CHANO)
V(SW_ANIME2CHANO)
V(SW_MESWINDOW0ALPHA)
V(SW_MESWINDOW1ALPHA)
V(SW_MESWINDOW2ALPHA)
V(SW_AUTOSAVERESTART)
V(SW_GAMESTATE)
V(SW_TITLEMODE)
V(SW_TITLEMOVIECT)
V(SW_TITLEDISPCT)
V(SW_SYSSEL)
V(SW_TITLEMASKALPHA)
V(SW_TITLEMASKCOLOR)
V(SW_TITLECGNO)
V(SW_TITLECUR)
V(SW_SYSTEMMENUCHG)
V(SW_SYSTEMMENUALPHA)
V(SW_SYSMENUCT)
V(SW_SYSMENUALPHA)
V(SW_SYSSUBMENUCT)
V(SW_SYSSUBMENUCTMAX)     // appeared in the LCC switch
V(SW_SYSMENUCTMAX)        // appeared in the LCC switch
V(SW_PIC_ARCHIVENO1)      // appeared in the LCC switch
V(SW_PIC_REQ_ARCHIVENO1)  // appeared in the LCC switch
V(SW_PIC_FILENO1)         // appeared in the LCC switch
V(SW_PIC_REQ_FILENO1)     // appeared in the LCC switch
V(SW_SYSSUBMENUNO)
V(SW_SYSSUBMENUALPHA)
V(SW_CLRALPHA)
V(SW_SYSMESCOL1)
V(SW_OPTIONCHAALPHA)
V(SW_OPTIONCHANO)
V(SW_TIMEYEAR)
V(SW_TIMEMONTH)
V(SW_TIMEDAY)
V(SW_TIMEHOUR)
V(SW_TIMEMINUTE)
V(SW_TIMESECOND)
V(SW_TIMEWEEK)
V(SW_LINEID)
V(SW_SCRIPTID)
V(SW_CAP1POSX_OFS)
V(SW_CAP1POSY_OFS)
V(SW_CAP1SX_OFS)
V(SW_CAP1SY_OFS)
V(SW_CAP1SIZE_OFS)
V(SW_CAP1LX_OFS)
V(SW_CAP1LY_OFS)
V(SW_CAP1ROTZ_OFS)
V(SW_CAP1ALPHA_OFS)
V(SW_MESNAMEID0)
V(SW_BG1POSX_OFS)
V(SW_BG1POSY_OFS)
V(SW_BG1SX_OFS)
V(SW_BG1SY_OFS)
V(SW_BG1SIZE_OFS)
V(SW_BG1LX_OFS)
V(SW_BG1LY_OFS)
V(SW_BG1ROTZ_OFS)
V(SW_BG1ALPHA_OFS)
V(SW_CHA1POSX_OFS)
V(SW_CHA1POSY_OFS)
V(SW_CHA1ROTZ_OFS)
V(SW_CHA1ROTX_OFS)
V(SW_CHA1ROTY_OFS)
V(SW_CHA1SIZEX_OFS)
V(SW_CHA1SIZEY_OFS)
V(SW_CHA1ALPHA_OFS)
V(SW_MESWIN0POSX_OFS)
V(SW_MESWIN0POSY_OFS)
V(SW_MASK1ALPHA_OFS)
V(SW_MASK2ALPHA_OFS)
V(SW_MASK3ALPHA_OFS)
V(SW_BGEFF1_OFSX)
V(SW_BGEFF1_OFSY)
V(SW_BGEFF1_SIZE_OFS)
V(SW_BGEFF1_ROTX_OFS)
V(SW_BGEFF1_ROTY_OFS)
V(SW_BGEFF1_ROTZ_OFS)
V(SW_BGEFF1_ALPHA_OFS)
V(SW_BGEFF1_SX_OFS)
V(SW_BGEFF1_SY_OFS)
V(SW_BGEFF1_MASK_VERTEX1_OFSX)
V(SW_BGEFF1_MASK_VERTEX1_OFSY)
V(SW_MAINTHDP)
V(SW_RESTARTMASK)
V(SW_PLATFORM)
V(SW_SAVEERRORCODE)
V(SW_SYSMENUCNO)
V(SW_TITLECUR1)
V(SW_TITLECUR2)
V(SW_SYSMESALPHA)
V(SW_SYSMESANIMCTCUR)
V(SW_SYSMESANIMCTF)
V(SW_SINSTALL_ALL)
V(SW_BG1SURF)
V(SW_MAP_PRI)
V(SW_MAP_ALPHA)
V(SW_DELUSION_OVERLAY_BUF)
V(SW_DELUSION_CIRCLE_BUF)
V(SW_DELUSION_STATE)
V(SW_DELUSION_BG_COUNTER)
V(SW_DELUSION_LIMIT)
V(SW_DELUSION_SPIN_COUNTER)
V(SW_DELUSION_PRI)
V(SW_YESNO_PRI)
V(SW_YESNO_SELECTED)
V(SW_YESNO_CT)
V(SW_YESNO_TARGET_INDEX)
V(SW_CHA1SURF)
V(SW_FACE1SURF)
V(SW_SHORTCUT)
V(SW_SVSENO)
V(SW_SVBGMNO)
V(SW_SVSCRNO1)
V(SW_SVSCRNO2)
V(SW_SVSCRNO3)
V(SW_SVSCRNO4)
V(SW_SVBGNO1)
V(SW_SVCHANO1)
V(SW_SVBGM2NO)
V(SW_SAVEFILESTATUS)
V(SW_SAVEFILENO)
V(SW_SAVEFILETYPE)
V(SW_TITLE)
V(SW_PLAYTIME)
V(SW_MESWINDOW_COLOR)
V(SW_BGMREQNO)
V(SW_SEREQNO)
V(SW_BGMVOL)
V(SW_SEVOL)
V(SW_SELNO)
V(SW_BGMREQNO2)
V(SW_SCRIPTNO0)
V(SW_SCRIPTNO1)
V(SW_SCRIPTNO2)
V(SW_SCRIPTNO3)
V(SW_SCRIPTNO4)
V(SW_SCRIPTNO5)
V(SW_SCRIPTNO6)
V(SW_SCRIPTNO7)
V(SW_SCRIPTNO8)
V(SW_SCRIPTNO9)
V(SW_SCRIPTNO10)
V(SW_SCRIPTNO11)
V(SW_SCRIPTNO12)
V(SW_SCRIPTNO13)
V(SW_SCRIPTNO14)
V(SW_SCRIPTNO15)
V(SW_MASK1COLOR)
V(SW_MASK1ALPHA)
V(SW_MASK1PRI)
V(SW_MASK1POSX)
V(SW_MASK1POSY)
V(SW_MASK1SIZEX)
V(SW_MASK1SIZEY)
V(SW_MESWIN0POSX)
V(SW_MESWIN0POSY)
V(SW_MESWIN0TYPE)
V(SW_MESMODE0)
V(SW_BGLINK)
V(SW_BGLINK2)
V(SW_BG1POSX)
V(SW_BG1POSY)
V(SW_BG1SX)
V(SW_BG1SY)
V(SW_BG1SIZE)
V(SW_BG1LX)
V(SW_BG1LY)
V(SW_BG1NO)
V(SW_BG1PRI)
V(SW_BG1DISPMODE)
V(SW_BG1FADECT)
V(SW_BG1FADETYPE)
V(SW_BG1ROTZ)
V(SW_BG1ALPHA)
V(SW_BG1MASKNO)
V(SW_BG1MASKFADERANGE)
V(SW_BG1CLIP_X)
V(SW_BG1CLIP_Y)
V(SW_BG1PRI2)
V(SW_BG1FILTER)
V(SW_BG1EFFPRI)
V(SW_BG1EFFPRI2)
V(SW_CAP1POSX)
V(SW_CAP1POSY)
V(SW_CAP1SX)
V(SW_CAP1SY)
V(SW_CAP1SIZE)
V(SW_CAP1LX)
V(SW_CAP1LY)
V(SW_CAP1NO)
V(SW_CAP1PRI)
V(SW_CAP1DISPMODE)
V(SW_CAP1FADECT)
V(SW_CAP1FADETYPE)
V(SW_CAP1ROTZ)
V(SW_CAP1ALPHA)
V(SW_CAP1MASKNO)
V(SW_CAP1MASKFADERANGE)
V(SW_CAP1PRI2)
V(SW_CAP1FILTER)
V(SW_FEATHERING_PRI)
V(SW_FEATHERING2_PRI)
V(SW_FEATHERING)
V(SW_FEATHERING2)
V(SW_MDL1POSX)
V(SW_MDL1POSY)
V(SW_MDL1POSZ)
V(SW_MDL1ROTX)
V(SW_MDL1ROTY)
V(SW_MDL1ROTZ)
V(SW_MDL1FACENO)
V(SW_MDL1ANIME)
V(SW_MDL1FILENO)
V(SW_MDL1ROTOY)
V(SW_MDL1CENY)
V(SW_MDL1TARDIR)
V(SW_MDL1CHANO)
V(SW_MDL1FACEDIR)
V(SW_MDL1FACEROT)
V(SW_MDL1FACEELV)
V(SW_MDL1FACESLA)
V(SW_MDL1ROT_INTER)
V(SW_MDL1FROT_INTER)
V(SW_MDL1POS_INTER)
V(SW_MDL1EYEU)
V(SW_MDL1EYEV)
V(SW_MDL1EYEROT)
V(SW_MDL1EYESIZE)
V(SW_MDL1EYEANIMENO)
V(SW_MDL1MOUTHANIMENO)
V(SW_MDL1OPTPARTS)
V(SW_MDL1LIGHTNO)
V(SW_CHA1POSX)
V(SW_CHA1POSY)
V(SW_CHA1ROTZ)
V(SW_CHA1ROTX)
V(SW_CHA1ROTY)
V(SW_CHA1SIZEX)
V(SW_CHA1SIZEY)
V(SW_CHA1ALPHA)
V(SW_CHA1FILTER)
V(SW_CHA1NO)
V(SW_CHA1PRI)
V(SW_CHA1POSE)
V(SW_CHA1FACE)
V(SW_CHA1EX)
V(SW_CHA1FADECT)
V(SW_CHA1FADETYPE)
V(SW_CHA1ANIME_EYE)
V(SW_CHA1ANIME_MOUTH)
V(SW_CHA1PRI2)
V(SW_CHA1BASESIZE)
V(SW_IRUOCAMERAPOSX)
V(SW_IRUOCAMERAPOSY)
V(SW_IRUOCAMERAPOSZ)
V(SW_IRUOCAMERAROTX)
V(SW_IRUOCAMERAROTY)
V(SW_CAMSHTARDIR)
V(SW_IRUOCAMERAHFOV)
V(SW_IRUOCAMERAHFOVDELTA)
V(SW_MAINCAMERAPOSX)
V(SW_MAINCAMERAPOSY)
V(SW_MAINCAMERAPOSZ)
V(SW_MAINCAMERAROTX)
V(SW_MAINCAMERAROTY)
V(SW_MAINCAMERAHFOV)
V(SW_MAINCAMERAHFOVDELTA)
V(SW_MAINLIGHTCOLOR)
V(SW_MAINLIGHTPOSX)
V(SW_MAINLIGHTPOSY)
V(SW_MAINLIGHTPOSZ)
V(SW_MAINLIGHTWEIGHT)
V(SW_MAINLIGHTDARKMODE)
V(SW_LIGHT1_COLORR)
V(SW_LIGHT1_COLORG)
V(SW_LIGHT1_COLORB)
V(SW_LIGHT1_POSX)
V(SW_LIGHT1_POSY)
V(SW_LIGHT1_POSZ)
V(SW_FACEEX1NO)
V(SW_FACEEX1FACE)
V(SW_FACEEX1FILTER)
V(SW_FACEPOSX)
V(SW_FACEPOSY)
V(SW_CAMSHROT_WORK)
V(SW_CAMSHELV_WORK)
V(SW_CAMSHPOSX_WORK)
V(SW_CAMSHPOSY_WORK)
V(SW_CAMSHPOSZ_WORK)
V(SW_CAMROT_WORK)
V(SW_CAMELV_WORK)
V(SW_CAMPOSX_WORK)
V(SW_CAMPOSY_WORK)
V(SW_CAMPOSZ_WORK)
V(SW_IRUOCAMERAHFOVCUR)
V(SW_MAINCAMERAHFOVCUR)
V(SW_MOSAIC)
V(SW_MOSAIC_PRI)
V(SW_EFF_WAVE_PRI)
V(SW_EFF_WAVE_ALPHA)
V(SW_MASK_CAPTURE_PRI)
V(SW_EFF_CAP_PRI)
V(SW_EFF_CAP_BUF)
V(SW_EFF_CAP_PRI2)
V(SW_EFF_CAP_BUF2)
V(SW_MOVIEPRI)
V(SW_ALPHAMOVIE_PRI)
V(SW_ALPHAMOVIE_ALPHA)
V(SW_MOVIE_PLAYNO)
V(SW_MOVIE_PLAYMODE)
V(SW_MOVIE_PLAYVIEW)
V(SW_MOVIE_LOADNO)
V(SW_ALPHAMOVIE_MODE)
V(SW_MOVIEALPHA)
V(SW_MOVIEPRI2)
V(SW_MOVIEALPHA2)
V(SW_MOVIEPRI3)
V(SW_MOVIEALPHA3)
V(SW_MOVIEPRI4)
V(SW_MOVIEALPHA4)
V(SW_MOVIEFRAME)
V(SW_MOVIETOTALFRAME)
V(SW_POKECON_OFSX)
V(SW_POKECON_OFSY)
V(SW_POKECON_PRI)
V(SW_POKECON_BOOTANIMECT)
V(SW_POKECON_SHUTDOWNANIMECT)
V(SW_POKECON_MENUCUR)
V(SW_POKECON_MENUSELANIMECT)
V(SW_POKECON_MODE)
V(SW_AR_POSX)
V(SW_AR_POSY)
V(SW_AR_ELV)
V(SW_PHONE_DISP_CT)
V(SW_AR_ELVMIN)
V(SW_AR_ELVMAX)
V(SW_AR_ROTMIN)
V(SW_AR_ROTMAX)
V(SW_POKECOMIRUOHFOV)
V(SW_BGEFF1_MASK_VERTEX1_X)
V(SW_BGEFF1_MASK_VERTEX1_Y)
V(SW_BGEFF1_SX)
V(SW_BGEFF1_SY)
V(SW_BGEFF1_SIZE)
V(SW_BGEFF1_PRI)
V(SW_BGEFF1_MASK_TYPE)
V(SW_BGEFF1_FADECT)
V(SW_BGEFF1_MODE)
V(SW_BGEFF1_ROTX)
V(SW_BGEFF1_ROTY)
V(SW_BGEFF1_ROTZ)
V(SW_BGEFF1_ALPHA)
V(SW_BGEFF1_MASKNO)
V(SW_BGEFF1_MASKFADERANGE)
V(SW_BGEFF1_PRI2)
V(SW_BGEFF1_FILTER)
V(SW_BGEFF1_POSX)
V(SW_BGEFF1_POSY)
V(SW_RENDERTARGET)
V(SW_INTROVOICE)
V(SW_UI_BTNGUIDE_REQ)
V(SW_UI_BTNGUIDE_PROG)
V(SW_UI_BTNGUIDE_TYPE)
V(SW_ATCHAN_SCROLL_MAX)
V(SW_MAP_ZOOM)
V(SW_MAP_POS_X)
V(SW_MAP_POS_Y)
V(SW_MAP_DST_POS_X)
V(SW_MAP_DST_POS_Y)
V(SW_MAP_SELECTED_POOL_INDEX)
V(SW_MAP_SELECTED_POOL_ID)
V(SW_MAP_SELECTED_POOL_TYPE)
V(SW_MAP_PHOTO_SELECT_STATE)
V(SW_MAP_RENDER_ALPHA_1)
V(SW_MAP_RENDER_ALPHA_2)
V(SW_MAP_RENDER_ALPHA_3)

// FlagWork
V(SF_CLR_FLAG)
V(SF_CLR_END1)
V(SF_CLR_TRUE_CC)
V(SF_CLR_TRUE_CCLCC)
V(SF_ACTORSVOICE_UNLOCK1)
V(SF_MOVIE_UNLOCK1)
V(SF_SYSTEMMENUDIRECT)
V(SF_MESSKIP)
V(SF_MESALLSKIP)
V(SF_MESREAD)
V(SF_MOVIE_DRAWWAIT)
V(SF_SELECTMODE)
V(SF_TITLEMODE)
V(SF_TITLEEND)
V(SF_UIHIDDEN)
V(SF_SHOWWAITICON)
V(SF_LOADING)
V(SF_BG1LOADEXEC)
V(SF_SAVEDISABLE)
V(SF_AUTOSAVEENABLE)
V(SF_MESREVDISABLE)
V(SF_MESSAVEPOINT_SSP)
V(SF_SYSMENUDISABLE)
V(SF_SYSTEMMENUDISABLE)
V(SF_SYSTEMMENUDISABLE2)
V(SF_GAMEPAUSE)
V(SF_ALBUMRELOAD)
V(SF_ALBUMEND)
V(SF_ALBUMCHA1)
V(SF_ALBUMLOAD_COMPLETE)
V(SF_ALBUMLOAD)
V(SF_LOADINGFROMSAVE)
V(SF_BACKLOG_NOLOG)
V(SF_BG1DISP)
V(SF_MDL1DISP)
V(SF_CHA1DISP)
V(SF_MDL1SHDISP)
V(SF_CAP1DISP)
V(SF_FACEEX1DISP)
V(SF_CAP1NOALPHACHK)
V(SF_BGEFF1DISP)
V(SF_BGEXPLOSIONVISIBLE)
V(SF_REVADDDISABLE)
V(SF_MOVIELOADPLAYFL)
V(SF_MASK_CAPTURE)
V(SF_MOVIEFL)
V(SF_MESWINDOW0OPENFL)
V(SF_CHA1BGEFFECT)
V(SF_Phone_Open)
V(SF_DATEDISPLAY)
V(SF_IRUOENABLE)
V(SF_MAPENABLE)
V(SF_MAP_RENDER_UNK1)
V(SF_MAP_RENDER_UNK2)
V(SF_MAP_RENDER_UNK3)
V(SF_MAP_RENDER_UNK4)
V(SF_MAP_NO_CLAMP)
V(SF_AR_SETUP_ADD_MDLBUF1)
V(SF_IRUOAUTO)
V(SF_IRUO)
V(SF_NEKOMIMIICON)
V(SF_DELUSIONACTIVE)
V(SF_DELUSIONSELECTED)
V(SF_DELUSION_UI_ANIM_WAIT)
V(SF_DELUSION_UI_ANIMSWITCH_WAIT)
V(SW_DELUSION_POS_TXT_IDX)      // CHLCC specific
V(SW_DELUSION_NEG_TXT_IDX)      // CHLCC specific
V(SW_MONITOR_SCANLINE_PRI)      // CHLCC specific
V(SW_MONITOR_SCANLINE_ENABLED)  // CHLCC specific
V(SW_EYECATCH_COUNT)            // CHLCC specific
V(SW_EYECATCH_BUF)              // CHLCC specific
V(SW_EYECATCH_PRI)              // CHLCC specific
V(SW_BUTTERFLY_COUNT)           // CHLCC specific
V(SW_BUTTERFLY_PRI)             // CHLCC specific
V(SW_BUTTERFLY_ALPHA)           // CHLCC specific
V(SW_BUBBLES_COUNT)             // CHLCC specific
V(SW_BUBBLES_PRI)               // CHLCC specific
V(SW_BUBBLES_ALPHA)             // CHLCC specific
V(SF_CLR_RIMI)                  // CHLCC specific
V(SF_CLR_NANAMI)                // CHLCC specific
V(SF_CLR_YUA)                   // CHLCC specific
V(SF_CLR_MIA)                   // CHLCC specific
V(SF_CLR_AYASE)                 // CHLCC specific
V(SF_CLR_SENA)                  // CHLCC specific
V(SF_CLR_KOZUE)                 // CHLCC specific
V(SF_CLR_SEIRA)                 // CHLCC specific
V(SW_TITLE_PRI)                 // CHLCC (or old version) specific
V(SF_ALLCLEAR)
V(SF_CONGRATULATED)
V(SF_Pokecon_Open)
V(SF_Pokecon_Cancel)
V(SF_Pokecon_End)
V(SF_Pokecon_ManualMode)
V(SF_Pokecon_Disable)
V(SF_SCN_CLR1)
V(SF_HELPMENU)
V(SF_BACKLOGMENU)
V(SF_SAVEMENU)
V(SF_SAVEPROTECTED)
V(SF_SAVEALLPROTECTED)
V(SF_SAVEPROTECTCHANGED)
V(SF_OPTIONMENU)
V(SF_SYSTEMMENU)
V(SF_TIPSMENU)
V(SF_ALBUMMENU)
V(SF_CLEARLISTMENU)
V(SF_SOUNDMENU)
V(SF_MOVIEMENU)
V(SF_RETURNTITLE)
V(SF_MOVIEPLAY)
V(SF_MOVIECANCEL)
V(SF_MESCLEAR0)
V(SF_RESTARTMASK)
V(SF_ACHIEVEMENTMENU)
V(SF_SYSTEMMENUCAPTURE)
V(SF_EXTRA_ENA)
V(SF_SUBMENUEXIT)
V(SF_SAVEICON)
V(SF_KEYWAITICONDISP)
V(SF_SAVECAPTURE)
V(SF_CHAANIME)