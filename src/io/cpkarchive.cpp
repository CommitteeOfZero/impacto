...
static std::string ReadString(int64_t stringsOffset, Stream* utfStream) {
  ...
  utfStream->Seek(stringAddr, SDL_IO_SEEK_SET);
  ...
  utfStream->Seek(retAddr, SDL_IO_SEEK_SET);
  return output;
}
...
static bool ReadUtfBlock(
  ...
  if (ReadBE<uint32_t>(utfStream.get()) != utfMagic) {
    ...
    utfStream->Seek(0, SDL_IO_SEEK_SET);
    if (ReadBE<uint32_t>(utfStream.get()) != utfMagic) {
  ...
  utfStream->Seek(4, SDL_IO_SEEK_CUR);
    ...
    switch (column.GetType()) {
      ...
      case CpkColumn::Kind::Data: {
        if (storeType == CpkColumn::Storage::DEFAULT) {
          ...
          utfStream->Seek(dataPos, SDL_IO_SEEK_SET);
  ...
  for (uint32_t i = 0; i < numRows; i++) {
    utfStream->Seek(rowsOffset + (i * rowLength), SDL_IO_SEEK_SET);
...
IoError CpkArchive::ReadItoc(int64_t itocOffset, int64_t contentOffset,
                             uint16_t align) {
  BaseStream->Seek(itocOffset, SDL_IO_SEEK_SET);
  ...
  BaseStream->Seek(4, SDL_IO_SEEK_CUR);
...
IoError CpkArchive::ReadToc(int64_t tocOffset, int64_t contentOffset) {
  BaseStream->Seek(tocOffset, SDL_IO_SEEK_SET);
...
IoError CpkArchive::ReadEtoc(int64_t etocOffset) {
  BaseStream->Seek(etocOffset, SDL_IO_SEEK_SET);
...
IoError CpkArchive::Open(FileMeta* file, Stream** outStream) {
  ...
  IoError err;
  BaseStream->Seek(entry->Offset, SDL_IO_SEEK_SET);
...
IoError CpkArchive::Slurp(FileMeta* file, void*& outBuffer, int64_t& outSize) {
  ...
  int64_t pos = BaseStream->Seek(entry->Offset, SDL_IO_SEEK_SET);
  if (pos != entry->Offset) {
    ImpLog(LogLevel::Error, LogChannel::IO,
