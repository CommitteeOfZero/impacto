...
IoError PhysicalFileStream::Create(std::string const& fileName, Stream** out,
                                   CreateFlags flags) {
  PhysicalFileStream* result = new PhysicalFileStream(GetSystemDependentPath(fileName), flags);
  ...
  if (!result->FileStream) {
    ImpLog(LL_Error, LC_IO, "Failed to open file \"%s\", error: \"%s\"\n",
           result->SourceFileName.c_str(), std::generic_category().message(errno).c_str());
    delete result;
    return IoError_Fail;
  }
