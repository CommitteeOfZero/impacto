...
void TipsMenu::SwitchToTipId(int id) {
  ...
  Category->SetText(
      ...
      CategoryFontSize, RendererOutlineMode::None, {TipsMenuDarkTextColor, 0});
  Name->SetText(
      {.ScriptBufferId = tipsScrBufId, .IpOffset = record->StringAdr[1]},
      NameFontSize, RendererOutlineMode::None, {TipsMenuDarkTextColor, 0});
  Pronounciation->SetText(
      {.ScriptBufferId = tipsScrBufId, .IpOffset = record->StringAdr[2]},
      PronounciationFontSize, RendererOutlineMode::None, 0);

  {
    ...
    float numberWidth = TextGetPlainLineWidth(
        stream, Profile::Dialogue::DialogueFont, NumberFontSize);
    Number->Bounds.X = NumberPos.x - numberWidth;
    Number->Bounds.Y = NumberPos.y;
    stream = Vm::Sc3Stream(sc3StringBuffer);
    Number->SetText(stream, NumberFontSize, RendererOutlineMode::None, 0);
  }
