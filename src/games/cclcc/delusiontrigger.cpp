...
void DelusionTrigger::Update(float dt) {
  if (!GetFlag(SF_DELUSIONACTIVE)) {
    if ((GetFlag(SF_MOVIEPLAY) && GetFlag(SF_MOVIE_DRAWWAIT)) ||
        ScrWork[SW_DELUSION_BG_COUNTER] == 0) {
      if (BgOverlaySprite.Sheet.Texture != nullptr) BgOverlaySprite = Sprite{};
      if (PositiveDelusionSprite.Sheet.Texture != nullptr)
        PositiveDelusionSprite.Sheet = SpriteSheet{};
      if (NegativeDelusionSprite.Sheet.Texture != nullptr)
    ...
    if (BgOverlaySprite.Sheet.Texture == nullptr) {
      int bgOverlayBGIdx =
