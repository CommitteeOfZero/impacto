...
void HelpMenu::UpdateInput(float dt) {
  ...
  bool prevBtnPressed =
      (PADinputButtonWentDown & PADcustom[38]) || Input::MouseWheelDeltaY < 0;
  if (State == Shown && prevBtnPressed) {
  ...
  bool nextBtnPressed =
      (PADinputButtonWentDown & PADcustom[39]) || Input::MouseWheelDeltaY > 0;
  if (State == Shown && nextBtnPressed) {
    PreviousPage = CurrentPage;
...
void HelpMenu::Render() {
  if (State != Hidden) {
    ...
    float topLeftX = FadeAnimation.Progress * 32 * 200 * 0.0625 - 400;
    ...
    if (PreviousPage > CurrentPage) {
      topLeftX = (1.0f - FadeAnimation.Progress) * 32 * 200 * 0.0625f - 400;
      Renderer->DrawSprite(
