...
void Bubble::Update(float dt) {
  // Direction and speed feels weird
  const float sine = sin(RandAngle / 65536.0f * 2 * std::numbers::pi_v<float>);
  Velocity.x = Size * (sine * 0.5f - 0.25f) + 0.5f;
  Velocity.y = -Size * 0.25f;
  RandAngle += 2048;
