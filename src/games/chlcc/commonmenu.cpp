...
void CommonMenu::DrawSubmenu(uint32_t backgroundColor,
  ...
  if (!GetFlag(SF_SYSTEMMENU)) {
    ...
    } else if (FromSystemMenuTransition.has_value() && GetFlag(SF_SYSTEMMENU)) {
      Renderer->DrawQuad(
          RectF(0.0f, 0.0f, Profile::DesignWidth, Profile::DesignHeight),
...
void CommonMenu::DrawCircles(Sprite& CurrentCircleSprite) const {
  ...
  const float scaleMultiplier = CurrentCircleSprite.Bounds.Height / 106.0f;
  for (int line = 0; line < 4; line++) {
    ...
    for (int col = 0; col < 7; col++) {
      if (counter + 1 <= progress) {
        ...
        Renderer->DrawSprite(
            CurrentCircleSprite,
            RectF(x + (CurrentCircleSprite.Bounds.Width - scale) / 2.0f,
                  y + (CurrentCircleSprite.Bounds.Height - scale) / 2.0f, scale,
                  scale));
        x += CircleOffset;
      }
      counter += 2;
...
void CommonMenu::DrawRightTitle(Sprite& titleSprite, float titleAngle) const {
...
void CommonMenu::DrawLeftTitle(Sprite& titleSprite, glm::vec4 tint) const {
  if (MenuTransition.Progress > 0.34f) {
...
void CommonMenu::UpdateTitles(glm::vec2 rightTitlepos, glm::vec2 leftTitlePos,
  ...
  const auto delta =
      glm::vec2(glm::mix(DiagonalTitlesOffsetStart.x, DiagonalTitlesOffsetEnd.x,
                         MenuTransition.Progress),
                glm::mix(DiagonalTitlesOffsetStart.y, DiagonalTitlesOffsetEnd.y,
                         MenuTransition.Progress));
