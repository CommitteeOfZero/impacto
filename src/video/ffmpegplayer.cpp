...
void FFmpegPlayer::InitSubtitles(
  ...
  for (auto& [subtitleStream, id] : embeddedSubStreams) {
    if (!subtitleStream.isSubtitle() || !subtitleStream.isValid()) continue;
    ...
    if (subtitleType == +SubtitleType::ASS) {
      ...
      SubPlayer->AddTrack(subStream.AvStream.id(), subtitleType, &subHeader);
    }
  }
  if (auto mappingsItr = SubtitleMappings.find(StreamPtr->Meta.FileName);
      mappingsItr != SubtitleMappings.end()) {
    initSubPlayer();
    int trackId = 0;
    for (auto const& [subType, subPath] : mappingsItr->second) {
      SubPlayer->AddTrackFile(trackId++, subType, subPath);
    }
  }
}
  ...
  if (IsPlaying) {
    ...
    if (AudioStream) {
      ...
      duration = Clock::Seconds(
          (double)VideoStream->AvStream.averageFrameRate().getDenominator() /
          VideoStream->AvStream.averageFrameRate().getNumerator());
    }
...
void FFmpegPlayer::UpdateSubtitles() {
  for (auto& subtitleStream : EmbeddedSubtitleStreams) {
    ...
    if (!hasSubtitle) {
      ImpLog(LogLevel::Info, LogChannel::Video, "No Subtitle");
    }
    ...
    for (unsigned rectI = 0; rectI < rawSubtitleData->num_rects; rectI++) {
      ...
      ImpLog(LogLevel::Info, LogChannel::Video,
             "\t\tRect {}: StartMs: {}, Duration: {}\n", rectI,
             subEntry.StartMs, subEntry.Duration);
    }
  }
}
